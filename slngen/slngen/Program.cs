...
namespace slngen
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
			...
			string versionOverride = Environment.GetEnvironmentVariable("SLNGEN_VERSION");
            ...
            foreach (var section in ParseConfig("slngen.conf"))
            {
                switch (section.Name.ToLowerInvariant())
                {
					...
					case "generateversion":
						section.Entries.TryGetValue("path",out versionFile);
						break;
                    default:
			...
			if(versionFile != null) {
				string versionFmt = "//Autogenerated\nusing System.Reflection;\n[assembly: AssemblyInformationalVersion(\"{0}\")]";
				string logV = versionOverride;
				var path = Path.Combine(basedir,versionFile);
				if(!string.IsNullOrEmpty(versionOverride)) {
					File.WriteAllText(path,string.Format(versionFmt,versionOverride));
				} else {
					string gitrev;
					if((gitrev = GitRevision(currentPlatform == BPlatforms.Windows)) != null) {
						logV = gitrev;
						File.WriteAllText(path,string.Format(versionFmt,gitrev));
					} else {
						logV = "0.0.1";
						File.WriteAllText(path,string.Format(versionFmt,"0.0.1"));
					}
				}
				Console.WriteLine("Generated version file: {0}, version {1}",path,logV);
			}
        }
		static string GitRevision(bool windows)
		{
			if(!Directory.Exists(".git")) {
				Console.Error.WriteLine("Warning: .git directory doesn't exist and SLNGEN_VERSION not set, defaulting to 0.0.1");
				return null;
			}
			if((windows && CommandExistsWin32("git")) || (!windows && CommandExistsUnix("git"))) {
				return GitRevParse().Trim() + "-git";
			} else {
				Console.Error.WriteLine("Warning: git command not found, reading master HEAD directly");
				byte[] bytes;
				using(var reader = new BinaryReader(File.OpenRead(".git/refs/heads/master"))) {
					bytes = reader.ReadBytes(7);
				}
				bool success = true;
				for(int i = 0; i < 7; i++) {
					if(!char.IsLetterOrDigit((char)bytes[i])) {
						Console.Error.WriteLine("Warning: reading ref failed, defaulting to 0.0.1");
						success = false;
						break;
					}
				}
				if(success)
					return System.Text.Encoding.ASCII.GetString(bytes) + "-git";
				else
					return null;
			}
		}
		public static string GitRevParse()
		{
		...
		public static bool CommandExistsUnix(string exeName)
		{
			Process p = new Process();
			p.StartInfo.UseShellExecute = false;
			p.StartInfo.FileName = "bash";
			p.StartInfo.Arguments = string.Format("-c 'command -v {0}'",exeName);
			p.StartInfo.RedirectStandardOutput = true;
			p.Start();
			p.WaitForExit();
			return p.ExitCode == 0;
		}
		public static bool CommandExistsWin32(string exeName)
		{
    		try
    		{ 
        		Process p = new Process();
    		...
    		{
				Console.Error.WriteLine("Warning: 'where' command invocation failed");
        		return false;
    		}   
		}
