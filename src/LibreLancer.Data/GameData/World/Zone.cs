...
public class Zone : NicknameItem
{
    ...
    private void UpdateShape(bool isPosition = false)
    {
        switch (data.Shape)
        {
            case ShapeKind.Box:
    ...
    public bool ContainsPoint(Vector3 point)
    {
        switch (data.Shape)
        {
            ...
            case ShapeKind.Ring:
            {
                ...
                var dsq = (pd.X * pd.X + pd.Y * pd.Y + pd.Z * pd.Z) - dot * dot / _shape.CylRing.SZ2.Y;
    ...
    private static bool StructsEqual<T>(ref T a, ref T b) where T : unmanaged
    {
    ...
    private static Vector2[] Convex2D(List<Vector2> points)
    {
        if (points == null || points.Count <= 1)
            throw new InvalidOperationException();
        int n = points.Count, k = 0;
