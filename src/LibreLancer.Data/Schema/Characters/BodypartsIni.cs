...
public class BodypartsIni
{
    ...
    {
        var candidates = Bodyparts.Where(b => b.Nickname.Equals(nickname, StringComparison.OrdinalIgnoreCase)).ToArray();
        var count = candidates.Length;
        return count switch
        {
            1 => candidates.First(),
            0 => null,
            _ => throw new Exception(count + " Bodyparts with nickname " + nickname)
        };
    }
    ...
    public Accessory? FindAccessory(string nickname)
    {
        var candidates = Accessories
            .Where(a => a.Nickname.Equals(nickname, StringComparison.OrdinalIgnoreCase)).ToArray();
        var count = candidates.Length;
        return count switch
        {
            1 => candidates.First(),
            0 => null,
            _ => throw new Exception(count + " Accessories with nickname " + nickname)
        };
    }
}
