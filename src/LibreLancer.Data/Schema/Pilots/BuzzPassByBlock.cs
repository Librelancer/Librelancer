...
public partial class BuzzPassByBlock : PilotBlock
{
    ...
    public List<DirectionWeight> BreakDirectionWeights = [];
    public List<BuzzPassByStyle> PassByStyleWeights = [];

    [EntryHandler("buzz_break_direction_weight", MinComponents = 2, Multiline = true)]
    private void HandleDirection(Entry e) => BreakDirectionWeights.Add(new DirectionWeight(e));

    [EntryHandler("buzz_pass_by_style_weight", MinComponents = 2, Multiline = true)]
    private void HandleStyle(Entry e) => PassByStyleWeights.Add(new BuzzPassByStyle(e));
}
