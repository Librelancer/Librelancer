...
public partial class EvadeBreakBlock : PilotBlock
{
    ...
    public List<DirectionWeight> DirectionWeights = [];
    public List<EvadeBreakStyle> StyleWeights = [];


    [EntryHandler("evade_break_style_weight", MinComponents = 2, Multiline = true)]
    private void HandleDodgeStyle(Entry e) => StyleWeights.Add(new EvadeBreakStyle(e));

    [EntryHandler("evade_break_direction_weight", MinComponents = 2, Multiline = true)]
    private void HandleDirectionWeight(Entry e) => DirectionWeights.Add(new DirectionWeight(e));
}
