...
public class ParallelActionRunner : IDisposable
{
    ...
    {
        managedWorker?.Invoke(workerIndex);
    ...
    private volatile Action<int>? managedWorker;
    ...
    private Counter remainingWorkerCounter;
    ...
    private void IndexWorker(int workerIndex)
    {
        int jobIndex;
        while ((jobIndex = Interlocked.Increment(ref currentTask)) < taskCount)
        {
            taskBody?.Invoke(jobIndex);
        }
    }

    private IList<Action>? tasks;

    private void ActionWorker(int workerIndex)
    {
        int jobIndex;
        while ((jobIndex = Interlocked.Increment(ref currentTask)) < taskCount)
        {
