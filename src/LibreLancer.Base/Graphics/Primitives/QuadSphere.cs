...
{
    private readonly ElementBuffer elementBuffer;

    private int primitiveCountSide;

    private Dictionary<CubeMapFace, int> offsets = new Dictionary<CubeMapFace, int>();

    public IVertexType VertexType => VertexBuffer.VertexType;
    ...
    private void TopBottom(int Y, int slices, VertexPositionNormalTexture[] vertices, int vertexCount)
    {
    ...
    private void FrontBack(int Z, int slices, VertexPositionNormalTexture[] vertices, int vertexCount)
    {
        int width = slices + 1, height = slices + 1;
        float advance = (2f / slices);
        float tadvance = (1f / slices);
    ...
    private void LeftRight(int X, int slices, VertexPositionNormalTexture[] vertices, int vertexCount)
    {
        int width = slices + 1, height = slices + 1;
        float advance = (2f / slices);
        float tadvance = (1f / slices);
    ...
    private void Indices(ushort t0, ushort t1, ushort t2, ushort t3, ushort t4, ushort t5, int slices, ref int i, ushort[] indices, int baseVert)
    {
        int width = slices + 1;
        int height = slices;
    ...
    private static Dictionary<CubeMapFace, Vector3> facePositions = new Dictionary<CubeMapFace, Vector3>()
    {
