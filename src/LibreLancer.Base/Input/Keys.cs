...
{
    private static SDL3.SDL_Scancode ToSDL3(Keys scancode)
    {
    ...
    {
        if (KeyNames.TryGetValue(k, out var n))
        {
            return n;
        }

        n = SDL3.Supported
            ? SDL3.SDL_GetKeyName(SDL3.SDL_GetKeyFromScancode(ToSDL3(k), 0, false))
            : SDL2.SDL_GetKeyName(SDL2.SDL_GetKeyFromScancode((SDL2.SDL_Scancode)k));

        KeyNames.Add(k, n);
        return n;
    }

    private static readonly SDL2.SDL_Keycode[] defaultMapping =
    {
