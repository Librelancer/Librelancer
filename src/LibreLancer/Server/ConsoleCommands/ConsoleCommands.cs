...
namespace LibreLancer.Server.ConsoleCommands
{
    public static class ConsoleCommands
    {
        ...
        public static bool ParseString<T>(string args, [MaybeNullWhen(false)] out T value)
        {
        ...
        public static bool ParseString<T1, T2>(string args, [MaybeNullWhen(false)] out T1 a,
            [MaybeNullWhen(false)] out T2 b)
        {
        ...
        public static bool ParseString<T1, T2, T3>(string args, [MaybeNullWhen(false)] out T1 a,
            [MaybeNullWhen(false)] out T2 b, [MaybeNullWhen(false)] out T3 c)
        {
        ...
        private static bool ParseString(string s, [MaybeNullWhen(false)] out object[] values, params Type[] types)
        {
            string[] split;
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                switch (T)
                {
                    ...
                    case Type _ when T == typeof(float):
                        if (!float.TryParse(split[i], NumberStyles.Any, CultureInfo.InvariantCulture,
                                out var floatValue))
                            return false;
                        values[i] = floatValue;
                        break;
