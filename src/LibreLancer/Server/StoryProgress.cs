...
public class StoryProgress
{
    ...
    public StoryItem CurrentStory = null!;
    ...
    public void Advance(Player player)
    {
        ...
        if (oldStory is { Skip: false, Acceptance: false })
        {
            player.LevelUp();
        }

        foreach (var x in oldStory.Actions.Where(x => x.Type == StoryActionType.AddRTC))
        {
        ...
        var loadMission = !string.Equals(oldStory.Mission, next.Mission, StringComparison.OrdinalIgnoreCase);
        CurrentStory = next;
