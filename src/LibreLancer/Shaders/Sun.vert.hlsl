...
struct VSInput
{
    ...
    [[vk::location(12)]] float4 color2: COLOR2;
...
cbuffer SunParameter : register(b3, UNIFORM_SPACE)
{
    float2 SizeMultiplier;
};
...
Output main(VSInput input)
{
    ...
    float2 dim = SizeMultiplier * input.dimensions.xy;
    ...
    output.outerColor = input.color2;
    return output;
}
