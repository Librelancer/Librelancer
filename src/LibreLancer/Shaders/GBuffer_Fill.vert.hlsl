// G-Buffer Fill Vertex Shader for Deferred Rendering
// Outputs geometry data to multiple render targets
...
Output main(VSInput input)
{
    ...
    // World space position for G-Buffer (deferred lighting needs world position)
    output.worldPosition = mul(float4(input.position, 1.0), World).xyz;

    // View space position (for depth reconstruction and effects)
    ...
    // Texture coordinates with material animation
