...
namespace LibreLancer.World.Components
{
    internal sealed class AutopilotObstacleAvoidance
    {
            ...
            GameObject? targetObject)
        {
            ...
            rays.Add(new(pos, pathDirection, probeLength), pool);
            rays.Add(new RayQuery(pos + right * halfWidth, pathDirection, probeLength), pool);
            rays.Add(new RayQuery(pos - right * halfWidth, pathDirection, probeLength), pool);
            rays.Add(new RayQuery(pos + up * halfHeight, pathDirection, probeLength), pool);
            rays.Add(new RayQuery(pos - up * halfHeight, pathDirection, probeLength), pool);
            for (int i = 0; i < candidates.Length; i++)
            {
                var origin = body.Position + right * candidates[i].Offset.X + up * candidates[i].Offset.Y;
                rays.Add(new RayQuery(origin, pathDirection, probeLength), pool);
            }

            world.Physics.BatchedRaycast(body, rays, hitObjects);
