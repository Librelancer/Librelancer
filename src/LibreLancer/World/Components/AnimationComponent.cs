...
namespace LibreLancer.World.Components
{
    public class AnimationComponent : GameComponent
    {
            ...
            float duration = 0, bool reverse = false)
        {
            ...
            {
                animations.Add(new ActiveAnimation(script, script.Name)
                {
        ...
        private bool ProcessAnimation(ActiveAnimation a)
        {
            ...
            {
                if (!ProcessJointMap(ref a.Script.JointMaps[i], ref a.JointCursors[i], a.WorldStartTime, ts, a.Loop,
                        a.Reverse))
                {
                    finished = false;
                }
        ...
        private bool ProcessJointMap(ref JointMap jm, ref int cur, double startTime, float timeScale, bool loop,
            bool reverse)
        {
            var mdl = Parent == null ? rm : Parent.Model!.RigidModel;
            var joint = mdl.Parts[jm.ChildName].Construct;
