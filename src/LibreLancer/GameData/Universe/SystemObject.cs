...
using LibreLancer.GameData.Characters;
...
namespace LibreLancer.GameData.Universe
{
	public class SystemObject : SystemPart, IDrawable
	{
		...
		private ElementBuffer atmosphereIndexBuffer;
		...
		public void Initialize(ResourceCache cache)
		{
			...
			if (AtmosphereRange != null)
			{
				if (Archetype is Planet)
				{
					...
					{
						Ellipsoid e = new Ellipsoid(new Vector3(AtmosphereRange.Value), 48, 64);
						atmosphereVertexBuffer = e.VertexBuffer;
						atmosphereIndexBuffer = e.ElementBuffer;
		...
		{
			Archetype.Update(camera);
			if (atmosphere) atmosphereMaterial.Update(camera);
		}
		...
		public void Draw(Matrix4 world, Lighting lights)
		{
			...
			Archetype.Draw (w, lights);

			if (atmosphere)
			{
				atmosphereMaterial.Render.World = w;
				atmosphereMaterial.Render.Use (
					atmosphereVertexBuffer.VertexType,
					lights
				);
				atmosphereVertexBuffer.Draw (PrimitiveTypes.TriangleList, 0, 0, atmosphereIndexBuffer.IndexCount / 3);
			}
		}
	}
}
