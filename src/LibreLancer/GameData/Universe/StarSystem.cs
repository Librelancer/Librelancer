	...
	public class StarSystem : UniverseElement, IDrawable
	{
		...
		public CmpFile BackgroundBasicStars
		{
			get
			{
				...
				if (backgroundBasicStars == null) backgroundBasicStars = new CmpFile(GameData.Freelancer.DataPath + backgroundBasicStarsPath, null);
			...
			get
			{
				...
				if (backgroundComplexStars == null)
				{
					if (backgroundComplexStarsPath.EndsWith(".cmp", StringComparison.OrdinalIgnoreCase))
						backgroundComplexStars = new CmpFile(GameData.Freelancer.DataPath + backgroundComplexStarsPath, null);
					else
						backgroundComplexStars = new ModelFile(GameData.Freelancer.DataPath + backgroundComplexStarsPath, null);
				}
		...
		public CmpFile BackgroundNebulae
		{
			get
			{
				...
				if (backgroundNebulae == null) backgroundNebulae = new CmpFile(GameData.Freelancer.DataPath + backgroundNebulaePath, null);
		...
		{
			this.camera = camera;
			if (BackgroundBasicStars != null) BackgroundBasicStars.Update(camera);
			if (BackgroundComplexStars != null) BackgroundComplexStars.Update(camera);
			if (BackgroundNebulae != null) BackgroundNebulae.Update(camera);

			foreach (SystemObject o in Objects) o.Update(camera);
		}

		public void Draw(Matrix4 world, Lighting lights)
		{
			//Color ambientColor = AmbientColor ?? ambient;
			lights.Ambient = AmbientColor ?? lights.Ambient;
			var starSphereWorld = Matrix4.CreateTranslation(camera.Position);
			if (BackgroundBasicStars != null) BackgroundBasicStars.Draw(starSphereWorld, lights);
			if (BackgroundComplexStars != null) BackgroundComplexStars.Draw(starSphereWorld, lights);
			if (BackgroundNebulae != null) BackgroundNebulae.Draw(starSphereWorld, lights);

			foreach (SystemObject o in Objects) o.Draw(world, lights);
		}
	}
}