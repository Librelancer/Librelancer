...
static ImGuiStorage FT_ColorIconMap;
static ImGuiStorage FT_ColorIconTint;

IMGUI_API void ImGuiFreeType::AddTintIcon(ImWchar codepoint, ImWchar icon, ImU32 color)
{
    FT_ColorIconMap.SetInt(codepoint, icon);
    FT_ColorIconTint.SetInt(codepoint, (int)color);
}
...
// CallumDev: Gamma correction table
    ...
    case FT_PIXEL_MODE_GRAY: // Grayscale image, 1 byte per pixel.
        {
            for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                for (uint32_t x = 0; x < w; x++)
                    dst[x] = IM_COL32(255, 255, 255, blurg_gamma[src[x]]);
            break;
        }
...
inline uint8_t FloatToPx(float f)
{
    if(f < 0.0)
        f = 0.0;
    if (f > 1.0)
        f = 1.0;
    return (uint8_t)(f * 255.0);
}
...
{
    ImGui_ImplFreeType_FontSrcData* bd_font_data = (ImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
    ImWchar renderCodepoint = (ImWchar)FT_ColorIconMap.GetInt(codepoint, codepoint);
    uint32_t glyph_index = FT_Get_Char_Index(bd_font_data->FtFace, renderCodepoint);
    if (glyph_index == 0)
        return false;
    ...
    const FT_Glyph_Metrics* metrics = ImGui_ImplFreeType_LoadGlyph(bd_font_data, renderCodepoint);
    if (metrics == nullptr)
        return false;
    ...
    if (is_visible)
    {
        ...
        // We are colouring an icon now
        if(renderCodepoint != codepoint)
        {
            ImU32 col = (ImU32)FT_ColorIconTint.GetInt(codepoint, 0xFFFFFFFF);
            float colA = ((col >> 24) & 0xFF)  / 255.0;
            float colR = ((col >> 16) & 0xFF) / 255.0;
            float colG = ((col >> 8) & 0xFF) / 255.0;
            float colB = (col & 0xFF) / 255.0;
            for(int i = 0; i < w * h; i++)
            {
                uint32_t px = temp_buffer[i];
                float pA = ((px >> 24) & 0xFF)  / 255.0;
                // ordering difference is intentional.
                // pixels are stored differently from imgui col32
                float pB = ((px >> 16) & 0xFF) / 255.0;
                float pG = ((px >> 8) & 0xFF) / 255.0;
                float pR = (px & 0xFF) / 255.0;
                pA *= colA;
                pR *= colR;
                pG *= colG;
                pB *= colB;
        ...
        out_glyph->Colored = (ft_bitmap->pixel_mode == FT_PIXEL_MODE_BGRA) || (renderCodepoint != codepoint);
...
bool ImGui_ImplFreetype_FontSrcContainsGlyph(ImFontAtlas* atlas, ImFontConfig* src, ImWchar codepoint)
{
    IM_UNUSED(atlas);
    ImGui_ImplFreeType_FontSrcData* bd_font_data = (ImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
    codepoint = (ImWchar)FT_ColorIconMap.GetInt(codepoint, codepoint);
