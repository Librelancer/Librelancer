...
{
    private delegate void AddPropertyAction(object o, List<PropertyEditor> editors);
    private static Dictionary<Type, List<AddPropertyAction>> constructors = new Dictionary<Type, List<AddPropertyAction>>();

    private static readonly string[] DefinedOrder =
    {
        "ID", "Anchor", "X", "Y", "Width", "Height", "Strid", "InfocardId"
    };

    private static int PropertyOrdering(PropertyInfo property)
    {
        var n = property.Name;
    ...
    private static void PopulateEditors(object o, List<PropertyEditor> editors)
    {
        var type = o.GetType();
