...
{
    private InterfaceResources resources;
    private UiData context;
    private MainWindow mainWindow;
    private TextBuffer colorName = new TextBuffer();
    private TextBuffer modelName = new TextBuffer();
    ...
    public bool Draw()
    {
        if (IsOpen)
        {
            ...
            {
                var path = context.FileSystem.GetBackingFileName(Path.Combine(context.XInterfacePath!,
                    "resources.xml"));
    ...
    private void ColorPickerSimple(InterfaceColor clr)
    {
    ...
    private void ColorPickerAnimated(InterfaceColor clr)
    {
    ...
    private void BindViewport(int szX, int szY)
    {
        if (szY <= 0)
        {
            szY = 1;
    ...
    private void DoViewport(InterfaceModel mdl)
    {
        if (drawable == null)
        {
            return;
    ...
    private void DoImagePreview(InterfaceImage mdl)
    {
        if (foundTexture == null)
        {
    ...
    private (Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4) GetQuad(float x, float y, float w, float h, float originX,
        float originY, float angle)
    {
        float dx = -originX;
        float dy = -originY;
