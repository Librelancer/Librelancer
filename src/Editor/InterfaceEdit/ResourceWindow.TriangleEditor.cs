...
{
    private void EditTriangle(InterfaceImage mdl)
    {
        ImGui.Text("Texture Coordinates");
        EditPoints("##texcoords", mdl.TexCoords,
            (x, y, w, h) => { ImGui.Image(foundTextureId, new Vector2(w, h), new Vector2(0, 1), new Vector2(1, 0)); },
    ...
    private void EditPoints(string label, InterfacePoints points, Action<int, int, int, int> renderResult,
        Point? pixels = null)
    {
        ImGui.PushID(label);
    ...
    private static float Snap(float a, float tolerance, float[] snaps)
    {
    ...
    private static uint ColorInt(Color4 c, float aMod = 1f)
    {
