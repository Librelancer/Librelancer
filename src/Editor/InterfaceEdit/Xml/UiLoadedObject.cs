...
public class LuaPrinterContext
{
    ...
    {
        return freeIdentifiers.Count > 0
            ? (freeIdentifiers.Dequeue(), false)
            : ($"_o{objNameIndex++:X}", true);
    }
...
public class UiLoadedObject
{
    ...
    public void Fill(object obj, List<XmlObjectMap>? maps = null)
    {
        if (maps != null)
        {
            foreach (var m in maps.Where(m => m.Object == this))
            {
                m.Object = obj;
                break;
            }
    ...
    private static ObjectActivator GetActivator(Type t)
    {
...
internal abstract class UiLoadedProperty
{
    private static Action<object, object> BuildSetter(Type targetType, PropertyInfo propertyInfo)
    {
    ...
    private static Dictionary<(Type t, PropertyInfo p), Action<object, object>> setters =
        new Dictionary<(Type t, PropertyInfo p), Action<object, object>>();

    private static object _setterLock = new object();

    private static Action<object, object> GetSetter(Type t, PropertyInfo p)
    {
        lock (_setterLock)
        {
            var key = (t, p);
...
internal class UiSimpleProperty : UiLoadedProperty
{
    public object Value;
...
internal class UiComplexProperty : UiLoadedProperty
{
...
internal class UiPrimitiveList : UiLoadedProperty
{
...
internal class UiPrimitiveDictionary : UiLoadedProperty
{
    public object[] Keys;
...
internal class UiComplexDictionary : UiLoadedProperty
{
    public readonly object[] Keys;
    public readonly UiLoadedObject[] Values;
    private readonly Type type;
...
internal class UiComplexList : UiLoadedProperty
{
