...
public class UiXmlWriter
{
    ...
    private static object GetWriteable(object input)
    {
        switch (input)
        {
            case InterfaceColor color when !string.IsNullOrEmpty(color.Name):
                return color.Name;
            case InterfaceColor color:
            {
