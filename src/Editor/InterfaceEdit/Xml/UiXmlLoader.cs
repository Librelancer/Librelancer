...
public class UiXmlLoader
{
    ...
    private static bool PrimitiveList(PropertyInfo property, XElement element, Type type,
        [MaybeNullWhen(false)] out UiPrimitiveList list)
    {
        list = null;
    ...
    private static bool CheckKeyType(Type type) => type.IsEnum || type.IsPrimitive || type == typeof(string);

    private static object GetDictionaryKey(XElement element, Type keyType)
    {
    ...
    private static bool PrimitiveDictionary(XElement element, PropertyInfo p, Type keyType, Type valueType,
        [MaybeNullWhen(false)] out UiPrimitiveDictionary primdict)
    {
        primdict = null;
        ...
        if (!valueType.IsEnum && !valueType.IsPrimitive || valueType != typeof(string))
        {
            return false;
        }
    ...
    {
        Dictionary<string, InterfaceColor> interfaceColors = Resources.Colors.ToDictionary(clr => clr.Name);
        Dictionary<string, InterfaceModel> interfaceModels = Resources.Models.ToDictionary(clr => clr.Name!);
        Dictionary<string, InterfaceImage> interfaceImages = Resources.Images.ToDictionary(clr => clr.Name);
    ...
    private UiLoadedObject ParseObjectInternal(
        Type objType,
        XElement el,
    ...
    )
    {
        ...
        foreach (var attr in el.Attributes())
        {
            ...
            if (attributes.TryGetValue(pname, out var property))
            {
                object? value = null;
                ...
                {
                    if (!interfaceColors.TryGetValue(attr.Value, out var color))
                    {
            ...
            if (property != null)
            {
                ...
                if (v is IList list)
                {
                    ...
                    else
                    {
                        ...
                        foreach (var itemXml in child.Elements())
                        {
                            ...
                            objs.Add(ParseObjectInternal(nt, itemXml, objectMaps, interfaceColors, interfaceModels,
                                interfaceImages));
                        }
                ...
                else if (v is IDictionary dict)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var nt = UiXmlReflection.Instantiate(itemXml.Name.ToString()).GetType();
                            values.Add(ParseObjectInternal(nt, itemXml, objectMaps, interfaceColors, interfaceModels,
                                interfaceImages));
