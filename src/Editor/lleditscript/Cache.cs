...
{
    private static readonly string MyId = typeof(Cache).Assembly.ManifestModule.ModuleVersionId.ToString();
    ...
    private static string GetCacheDir()
    {
        ...
        var osConfigDir = Environment.GetEnvironmentVariable("XDG_CACHE_HOME");
    ...
    {
        var mutexName = $"Global\\{Environment.UserName}_{Environment.UserDomainName}_lleditscript";
    ...
    private static string CacheID(string code)
    {
    ...
    private record ManifestItem(string Name, long Size, DateTime Created);
        ...
        WithMutex(() =>
        {
            ...
            try
            {
                if(File.Exists(manifestPath))
                {
                    ...
                    manifest = JsonSerializer.Deserialize<Manifest>(f) ?? throw new InvalidOperationException();
                }
            }
            catch (Exception)
            {
