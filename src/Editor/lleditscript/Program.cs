...
public class Globals //Must be public or compiler error in script
    (string?[] args, string? scriptName)
{
    public readonly string?[] Arguments = args;

    private string usage = "";

    public void ScriptUsage(string newUsage)
    {
        this.usage = newUsage;
    }
    ...
    {
        Console.WriteLine(!string.IsNullOrEmpty(usage)
            ? $"Usage: lleditscript {scriptName} {usage}"
            : $"Usage: lleditscript {scriptName} [options] [arguments]");
...
internal class Program
{
    ...
    private static string AssemblyEditorScriptPath()
    {
        return Path.Combine(Path.GetDirectoryName(typeof(Program).Assembly.Location) ?? AppDomain.CurrentDomain.BaseDirectory, "editorscripts");
    }

    private static string ModuleEditorScriptPath()
    {
    ...
    private static string? SearchFile(params string?[] files) =>
        files.FirstOrDefault(File.Exists, files[^1]);

    private static Script<object>? CompileScript(string? filePath, string scriptText)
    {
        ...
        return result.Any(x => x.Severity == DiagnosticSeverity.Error) ? null : script;
    }

    private static int RunAssembly(Assembly scriptAssembly, Globals globals)
    {
        ...
        var tk = (Task<object>?)factory?.Invoke(null, [submissionArray]);
        tk?.Wait();
    ...
    private static int Main(string?[] args)
    {
        ...
        if (argsStdin)
        {
            ...
            scriptArguments = inputArgs.ToArray();
        } else
        {
        ...
        var scriptText = ReadScript(filePath);
