...
public static unsafe class ImGuiExt
{
    ...
    public static extern bool SplitterV(float thickness, ref float size1, ref float size2, float min_size1,
    ...
    public static unsafe void ToastText(string text, Color4 background, Color4 foreground)
    {
        ...
        drawlist.AddRectFilled(new Vector2(displaySize.X - textSize.X - 9, 2),
            new Vector2(displaySize.X, textSize.Y + 9), GetUint(background), 2, ImDrawFlags.RoundCornersAll);
        drawlist.AddText(new Vector2(displaySize.X - textSize.X - 3, 2), GetUint(foreground), text);
    }
    ...
    public static bool InputTextLogged(string id, ref string buf, int bufSize, Action<string, string> onChanged,
        bool inputId = false)
    {
        ...
        if (ImGui.IsItemDeactivatedAfterEdit() && originalString != buf)
        {
