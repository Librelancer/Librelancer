...
public unsafe class AudioManager
{
    ...
    internal readonly IUIThread UIThread;
    ...
    private void InitSourceProperties(uint src, ref SoundInstance.SourceProperties prop, SoundCategory category)
    {
    ...
    private bool InRange(SoundInstance instance)
    {
    ...
    private void AlSetListenerOrientation(Vector3 forward, Vector3 up)
    {
    ...
    private uint BufferData(SoundData data)
    {
        var id = Al.GenBuffer();
        data.Reference();
    ...
    private static void UnBufferData(uint buffer, SoundData data)
    {
        Al.alDeleteBuffers(1, ref buffer);
        data.Dereference();
    }

    private void StartSource(SoundInstance instance, float offset, bool looping)
    {
    ...
    private void StopSource(SoundInstance instance)
    {
        if (instance.Source != -1)
        {
            uint sid = (uint)instance.Source;
    ...
    private void InstanceStopped(SoundInstance snd)
    {
        snd.Active = false;
    ...
    private Stopwatch audioClock = null!;
    private TimeSpan timeStep = TimeSpan.FromTicks(166667);
