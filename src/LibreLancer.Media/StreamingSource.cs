...
internal class StreamingSource : IDisposable
{
    private const int POOL_BUFFER_SIZE = 8192;
    private bool dataLeft;
    private readonly StreamingSound sound;
    private uint ID;
    private bool looping = false;
    private bool playing = false;
    public EventHandler? Stopped;
    private readonly MusicPlayer manager;
    ...
    public void Begin(bool loop)
    {
        ...
        for (var i = 0; i < 8; i++) {
            ...
            var read = Read(bytes, sound.Data);
            if (read != 0)
            {
                try
                {
    ...
    private int Read(byte[] buffer, Stream stream)
    {
        var read = stream.Read(buffer, 0, POOL_BUFFER_SIZE);
        if (read == POOL_BUFFER_SIZE || read == 0 || read % 4 == 0) return read;
        var r2 = stream.Read(buffer, read, read % 4);
        if (r2 != 0) {
            read += r2;
        }
    ...
    public bool Update()
    {
        ...
        if (val is Al.AL_PLAYING or Al.AL_PAUSED)
        {
            return true;
        }
    ...
    private void Cleanup()
    {
        ...
        Al.alGetSourcei(ID, Al.AL_BUFFERS_PROCESSED, out var p);
