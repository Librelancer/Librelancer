...
{
    private StreamingSource? sound;
    private StreamingSource? oldSound;
    ...
    private float GetGain(double amount, float attenuation)
    {
    ...
    {
        sound?.Gain = crossFadeDuration > 0
            ? GetGain(crossFadeTime / crossFadeDuration, currentAttenuation)
            : GetGain(1, currentAttenuation);
    }
